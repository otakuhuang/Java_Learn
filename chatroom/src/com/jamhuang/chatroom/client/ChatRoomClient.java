...
import com.jamhuang.chatroom.common.Utils;
        ...
        Thread readThread = new Thread(() -> {
            while (true) {
                try {
                    ...
                    if (chatMessage.getFrom().equalsIgnoreCase(ADMIN_NAME) && chatMessage.getMessage().trim().equalsIgnoreCase(BYE)) {
                        exchange.close();
                        System.out.println("已经离开聊天室，程序结束");
                        System.exit(0);
                    }
                    System.out.printf("from \"" + chatMessage.getFrom() + "\":" + chatMessage.getMessage());
                } catch (IOException e) {
                    e.printStackTrace();
                    exchange.close();
                    System.exit(-2);
